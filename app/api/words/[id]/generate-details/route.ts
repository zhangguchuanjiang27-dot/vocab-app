...
export async function POST(
    ...
    try {
        ...
        const prompt = `
            ...
            1. examples
               - この単語「${word.word}」について、例文を生成してください。
               
               【厳格な生成ルール】
               Step 1. **意味の特定と選定（最大3つ）**:
                  - まず、ユーザーが登録した意味「${word.meaning}」を確認し、これを最優先でリストに加えます。
                  - 次に、この単語が持つ他の**重要な意味（異なる品詞やニュアンス）**があれば、それをリストに追加してください。
                  - **リストの上限は最大3つまで**です。重要度の低い意味は切り捨ててください。

               Step 2. **例文の作成（意味の数と一致させる）**:
                  - 選定した**それぞれの意味に対して、必ず1つずつ**例文を作成してください。
                  - 例：意味が1つだけなら例文は1つ。意味が3つ選ばれたなら例文は3つです。
                  - **同じ意味に対して複数の例文を作らないでください。**

               Step 3. **出力形式**:
                  - roleは必ず**【品詞（具体的な意味）】**の形式。（例: "動詞（走る）"）
                  - 単語単体（例: "run"）の場合、その単語自体の意味の例文のみ。熟語は含めない。
