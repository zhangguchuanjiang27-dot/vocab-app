    ...
    try {
        ...
        const prompt = `
            ...
            1. weather
               ...
               【生成ルール】
               1. まず、ユーザーが登録した意味「${word.meaning}」を分析してください。もしこれが複数の意味（「走る、経営する」など）を含んでいる場合、それらを**別の項目として分割**して例文を作ってください。
               2. それでも数が足りない場合（5つ未満の場合）、この単語が持つ他の一般的な意味・用法（品詞違いや派生義）を補って、**合計3〜5個**になるようにしてください。
               3. **必ず複数の例文配列**を返してください。1つだけで終わらせないでください。
