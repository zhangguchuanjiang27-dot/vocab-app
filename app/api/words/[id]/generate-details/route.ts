...
export async function POST(
    ...
    try {
        ...
        【作成ルール】
        1. **意味ごとの生成**: 意味に含まれる各要素（例：作る、創造する）に対して、個別に例文を作成。
        2. **roleの形式**: 必ず **「品詞(意味)」** としてください。
           ※品詞は (動詞、名詞、形容詞、副詞、熟語、他) のように、略さず正式名称で書いてください。
           例: "動詞(作る)", "名詞(首都)", "形容詞(主要な)"
        3. **品質**: 自然で実用的な英語（10-15語以上）。
        4. **単語の無変化（重要）**: 対象の単語（word）は、形や時制（複数形、過去形、進行形など）を**一切変更せず**、完全に同じスペルのままで例文の中に使用してください（例: playが入力ならplayedやplaysにせずplayのまま使う）。
