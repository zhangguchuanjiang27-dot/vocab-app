    ...
    const prompt = `
      ...
      4. **例文の生成（必須 - 重要）**:
         - **examples**: この単語の代表的な使い方を示す例文を**必ず**作成してください。
           - role: **【品詞（具体的な意味）】**の形式で出力してください (例: "動詞（走る）", "熟語（立候補する）")。
           - text: 英文
           - translation: 和訳
           - 1〜2個生成してください。
           - **品質**: 短すぎる文や汎用的な文（例: "I like it"）は避け、**文脈が豊かで、単語のニュアンスが伝わる自然な例文**にしてください。
           - **類義語・対義語は生成しないでください。**
           - **重要: 入力が熟語（例: "run for", "take off"）の場合、roleには必ず「熟語（〜）」と記述してください。**
