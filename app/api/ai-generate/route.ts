...
export async function POST(req: Request) {
  ...
  try {
    ...
    const normalizationPrompt = `
      ...
      3. **単語の正規化（重要）**:
         - 基本的には辞書形（原形）に直しますが、**その形で固有の意味を持つ名詞**はそのままにしてください。
         - **複数形で意味を持つ名詞**: binoculars（双眼鏡）、glasses（眼鏡）、goods（商品）、clothes（服）などは、sを取らずに**そのまま出力**してください。 "binocular" や "glass" にすると意味が変わるためです。
         - **~ing形で意味を持つ名詞**: drawing（抽選、図面）、meeting（会議）、building（建物）などは、ingを取らずに**そのまま出力**してください。 "draw" や "meet" にすると動詞になってしまうためです。
         - 単なる動詞の活用（例: "playing" -> "play"）や、単純な複数形（例: "cats" -> "cat"）は原形に戻してください。
