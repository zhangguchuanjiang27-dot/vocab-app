      ...
      const prompt = `
          ...
          - meaning: 日本語の意味。形式は「【品詞】意味1、意味2...」としてください。
            **重要: 英単語やカタカナ英語（英語の読みをそのままカタカナにしたもの）は絶対に含めないでください。純粋な日本語訳のみを出力してください。**
            例: "capital" なら "【名】首都、資本、大文字 【形】主要な、重要な" (× "【名】キャピタル")
      ...
      if (newWordsData.length > 0) {
        try {
          const examplesPrompt = `
                  ...
                  【作成ルール】
                  1. **意味の完全分割**: "meaning" に複数の意味が含まれている場合（例: "【名】首都、資本"）、**必ずそれぞれの意味ごとに個別の項目を作成**してください。
                     ❌ 絶対にやってはいけない: role: "名詞(首都、資本)" のようにまとめること。
                     ✅ 正しい出力: role: "名詞(首都)" と role: "名詞(資本)" の2つの項目に分ける。

                  2. **roleの形式**: 必ず **「品詞(単一の意味)」** としてください。
                     例: "名詞(首都)", "名詞(資本)", "形容詞(重要な)"
                    ...
                    "details": [
                      {
                        ...
                        "otherExamples": [
                          ...
                          { "role": "名詞(大文字)", "text": "Please write your name in capital letters.", "translation": "名前は大文字で書いてください。" },
                          { "role": "形容詞(重要な)", "text": "It was a capital mistake.", "translation": "それは重大な間違いでした。" }
                        ]
                      }
                    ]
                  }
