...
export async function POST(req: Request) {
  ...
  try {
    ...
    const normalizationPrompt = `
      ...
      4. **スペルミスの修正 (Spelling Correction) [重要]**:
         入力された単語に明らかなスペルミスやタイポがある場合（例: "patroniza" → "patronize"、"aple" → "apple"）、正しいスペルに修正してから出力してください。
         ただし、意図的な造語や固有名詞と思われる場合は無理に修正しないでください。

      5. **入力数と出力数の一致 (One-to-One)**:
        ...
        for (let i = 0; i < newWordsData.length; i += CHUNK_SIZE) {
          ...
          try {
            const examplesPrompt = `
                    ...
                    6. **単語の無変化（重要）**: 対象の単語（word）は、形や時制（複数形、過去形、進行形など）を**一切変更せず**、完全に同じスペルのままで例文の中に使用してください（例: playが入力ならplayedやplaysにせずplayのまま使う）。
